...
const SPONSOR_TIERS = [
  ...
  { value: 'possible',  label: '◎ Eligible Sponsors' },
];
...
function getSalaryLabel(job) {
  ...
  if (t.includes('junior') || t.includes('graduate') || t.includes('trainee')) return { text: 'Typically £25k–£35k', real: false };
...
function getCoSRoute(job) {
  // Use stored sector if available, fall back to title matching
  if (job.sector === 'Healthcare & Medical') return 'Health & Care';
  if (job.sector === 'Education') return 'Education';
  const t = (job.title || '').toLowerCase();
...
export default function Jobs() {
  ...
  const [sectors, setSectors]         = useState(searchParams.get('sectors') ? searchParams.get('sectors').split(',') : []);
  const [workTypes, setWorkTypes]     = useState(searchParams.get('work') ? searchParams.get('work').split(',') : []);
  const [sponsorTier, setSponsorTier] = useState(searchParams.get('tier') || 'all');
  const [visaRoute, setVisaRoute]     = useState(searchParams.get('route') || 'all');
  ...
  useEffect(() => {
    ...
    if (workTypes.length > 0)   params.work    = workTypes.join(',');
    if (sponsorTier !== 'all')  params.tier    = sponsorTier;
    if (visaRoute !== 'all')    params.route   = visaRoute;
    if (sort !== 'newest')      params.sort    = sort;
    if (salaryMin)              params.smin    = salaryMin;
    if (salaryMax)              params.smax    = salaryMax;
    if (page > 1)               params.page    = page;
    setSearchParams(params, { replace: true });
  }, [search, location, sectors, workTypes, sponsorTier, visaRoute, sort, salaryMin, salaryMax, page, isPaid, setSearchParams]);
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if (isPaid) {
        ...
        if (search.trim()) {
          query = query.or([
            ...
            `company_name.ilike.%${search.trim()}%`,
          ].join(','));
        }
        ...
        // Sector — now uses stored sector column, not keyword matching
        if (sectors.length > 0) {
          query = query.in('sector', sectors);
        }

        // Visa route — maps to stored sector
        if (visaRoute !== 'all') {
          if (visaRoute === 'Health & Care') {
            query = query.eq('sector', 'Healthcare & Medical');
          } else if (visaRoute === 'Education') {
            query = query.eq('sector', 'Education');
          } else if (visaRoute === 'Skilled Worker') {
            query = query.not('sector', 'in', '("Healthcare & Medical","Education")');
          }
        }
        ...
        query = query.order('posted_at', { ascending: false }).limit(FREE_LIMIT);
      }
      ...
      let results = data || [];
      if (isPaid && workTypes.length > 0) {
        results = results.filter(j => workTypes.includes(detectWorkType(j.description)));
      }
    ...
    setLoading(false);
  }, [authLoading, isPaid, search, location, sectors, workTypes, sponsorTier, visaRoute, sort, salaryMin, salaryMax, page]);

  useEffect(() => { const t = setTimeout(fetchJobs, 300); return () => clearTimeout(t); }, [fetchJobs]);
  useEffect(() => { setPage(1); }, [search, location, sectors, workTypes, sponsorTier, visaRoute, sort, salaryMin, salaryMax]);
  useEffect(() => {
      ...
      <div style={{ background: '#080808', minHeight: '100vh', overflowX: 'hidden' }}>
        ...
        <div className="page-inner" style={{ paddingTop: '6rem', paddingBottom: 0 }}>
          ...
          <div className="pc-search-row">
            <input className="filter-input search-input" placeholder="Job title, skill or keyword..." value={search}
              onChange={isPaid ? e => setSearch(e.target.value) : undefined}
              {...(!isPaid ? { readOnly: true, onClick: () => handleLocked('search'), style: { cursor: 'not-allowed', opacity: 0.45 } } : {})} />
            <input className="filter-input location-input" placeholder="Location..." value={location}
              onChange={isPaid ? e => setLocation(e.target.value) : undefined}
              {...(!isPaid ? { readOnly: true, onClick: () => handleLocked('location'), style: { cursor: 'not-allowed', opacity: 0.45 } } : {})} />
          ...
          <div className="mobile-only" style={{ display: 'flex', gap: '0.6rem', marginBottom: '1rem', width: '100%' }}>
            <input className="filter-input" placeholder="Job title, skill or keyword..." value={search}
              onChange={isPaid ? e => setSearch(e.target.value) : undefined}
              {...(!isPaid ? { readOnly: true, onClick: () => handleLocked('search'), style: { flex: 3, cursor: 'not-allowed', opacity: 0.45 } } : { style: { flex: 3 } })} />
            <input className="filter-input" placeholder="Location..." value={location}
              onChange={isPaid ? e => setLocation(e.target.value) : undefined}
              {...(!isPaid ? { readOnly: true, onClick: () => handleLocked('location'), style: { flex: 2, cursor: 'not-allowed', opacity: 0.45 } } : { style: { flex: 2 } })} />
          </div>
            ...
            <aside className="desktop-sidebar" style={{ width: 232, flexShrink: 0, position: 'sticky', top: '5.5rem' }}>
              ...
              <div style={{ marginBottom: '1.6rem' }}>
                ...
                <div ref={sectorDropRef} style={{ position: 'relative' }}>
                  <button onClick={isPaid ? () => setSectorDropOpen(o => !o) : () => handleLocked('sector')}
                    className={shakeTarget === 'sector' ? 'do-shake' : ''}
                    style={{ width: '100%', textAlign: 'left', background: 'rgba(240,237,232,0.06)', border: '1px solid rgba(240,237,232,0.1)', borderRadius: 10, padding: '0.62rem 2.2rem 0.62rem 0.9rem', color: 'rgba(240,237,232,0.45)', fontSize: '0.82rem', cursor: isPaid ? 'pointer' : 'not-allowed', fontFamily: 'inherit', position: 'relative', opacity: isPaid ? 1 : 0.45 }}>
              ...
              {/* Visa Route */}
              <div style={{ marginBottom: '1.6rem' }}>
                <span className="sidebar-label">Visa route {!isPaid && <span style={{ color: '#c8ff00', fontSize: '0.55rem' }}>· Monthly</span>}</span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                  {VISA_ROUTES.map(r => (
                    <button key={r.value}
                      onClick={isPaid ? () => setVisaRoute(r.value) : () => handleLocked('route')}
                      className={`sidebar-filter-btn${visaRoute === r.value && isPaid ? ' active' : ''}${!isPaid ? ' locked' : ''} ${shakeTarget === 'route' ? 'do-shake' : ''}`}
                    >{r.label}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: '1.6rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                  {WORK_TYPES.map(w => (
                    <button key={w}
                      onClick={isPaid ? () => setWorkTypes(prev => prev.includes(w) ? prev.filter(x => x !== w) : [...prev, w]) : () => handleLocked('worktype')}
                      className={`sidebar-filter-btn${workTypes.includes(w) && isPaid ? ' active' : ''}${!isPaid ? ' locked' : ''} ${shakeTarget === 'worktype' ? 'do-shake' : ''}`}
                    >{w}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: '1.6rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                  {SPONSOR_TIERS.map(t => (
                    <button key={t.value}
                      onClick={isPaid ? () => setSponsorTier(t.value) : () => handleLocked('sponsor')}
                      className={`sidebar-filter-btn${sponsorTier === t.value && isPaid ? ' active' : ''}${!isPaid ? ' locked' : ''} ${shakeTarget === 'sponsor' ? 'do-shake' : ''}`}
                    >{t.label}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: '1.6rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <input className="filter-input" placeholder="Min £" type="number" value={salaryMin}
                    onChange={isPaid ? e => setSalaryMin(e.target.value) : undefined}
                    {...(!isPaid ? { readOnly: true, onClick: () => handleLocked('salary') } : {})}
                    style={{ width: '50%', opacity: isPaid ? 1 : 0.38, cursor: isPaid ? 'text' : 'not-allowed' }} />
                  <input className="filter-input" placeholder="Max £" type="number" value={salaryMax}
                    onChange={isPaid ? e => setSalaryMax(e.target.value) : undefined}
                    {...(!isPaid ? { readOnly: true, onClick: () => handleLocked('salary') } : {})}
                    style={{ width: '50%', opacity: isPaid ? 1 : 0.38, cursor: isPaid ? 'text' : 'not-allowed' }} />
                </div>
              </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                    {jobs.map(job => {
                      ...
                      return (
                        <div key={job.id} className="job-card"
                          ...
                          style={{ background: '#111', border: '1px solid rgba(240,237,232,0.07)', borderRadius: 20, padding: '1.25rem 1.4rem', width: '100%', overflow: 'hidden' }}>
                          ...
                          <div style={{ marginBottom: '0.6rem' }}>
                            {salary.real
                              ? <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '0.92rem', color: '#c8ff00' }}>{salary.text}</span>
                              : <span style={{ fontSize: '0.78rem', color: 'rgba(240,237,232,0.28)' }}>{salary.text}</span>
                            }
                          </div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
                              {job.sponsorship_tier === 'confirmed'
                                ...
                                : <span style={{ background: 'transparent', color: 'rgba(240,237,232,0.3)', fontSize: '0.58rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.22rem 0.6rem', borderRadius: '100px', border: '1px solid rgba(240,237,232,0.1)', whiteSpace: 'nowrap' }}>◎ Eligible Sponsor</span>
                              }
                    ...
                    {jobs.length === 0 && !loading && !error && (
                      <div style={{ textAlign: 'center', padding: '4rem 2rem' }}>
                        ...
                        <div style={{ fontSize: '0.82rem', color: 'rgba(240,237,232,0.28)' }}>Try different keywords or clear your filters.</div>
                      </div>
                    )}
                  </div>
                  ...
                  {!isPaid && jobs.length > 0 && (
                    <div className="upgrade-banner">
                      ...
                      <p style={{ fontSize: '0.84rem', color: 'rgba(240,237,232,0.45)', lineHeight: 1.65, marginBottom: '1.5rem', maxWidth: 420, margin: '0 auto 1.5rem' }}>
                        Search by role, location, salary and sector. Filter by visa route and confirmed sponsors only.
                      </p>
        ...
        <div style={{ width: '100%', padding: '1rem 2rem', borderTop: '1px solid rgba(240,237,232,0.05)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', gap: '0.7rem', alignItems: 'flex-start' }}>
            ...
            <p style={{ fontSize: '0.72rem', color: 'rgba(240,237,232,0.2)', lineHeight: 1.65, margin: 0 }}>
              Job listings on Sponsrr are sourced automatically from hundreds of recruiter databases and updated every hour. While we do our best to keep information accurate, we can sometimes fail - incorrect salary range, outdated listings, or mismatched sponsor status can occasionally occur. This is not us getting things wrong, it's the data provided to us by third-party sources. Always verify sponsorship directly with the employer.
            </p>
          </div>
        </div>
          ...
          <>
            ...
            <div className="filter-sheet">
              ...
              <div style={{ display: 'flex', gap: '0.45rem', marginBottom: '1.4rem', flexWrap: 'wrap' }}>
                {VISA_ROUTES.map(r => (
                  <button key={r.value}
                    onClick={isPaid ? () => setVisaRoute(r.value) : () => { setSidebarOpen(false); setShowModal(true); }}
                    style={{ background: visaRoute === r.value && isPaid ? 'rgba(200,255,0,0.1)' : 'rgba(240,237,232,0.07)', color: visaRoute === r.value && isPaid ? '#c8ff00' : 'rgba(240,237,232,0.6)', border: visaRoute === r.value && isPaid ? '1px solid rgba(200,255,0,0.25)' : '1px solid rgba(240,237,232,0.1)', borderRadius: 100, padding: '0.42rem 1rem', fontSize: '0.78rem', fontWeight: visaRoute === r.value && isPaid ? 600 : 400, cursor: 'pointer', fontFamily: 'inherit', opacity: isPaid ? 1 : 0.45 }}
                  >{r.label}</button>
                ))}
              </div>
